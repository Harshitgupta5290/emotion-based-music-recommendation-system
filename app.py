...
face_classifier = cv2.CascadeClassifier('/home/harshit/Downloads/EMOTION-BASED-MUSIC-AND-VIDEO-RECOMMENDATION-SYSTEM-master/haarcascade_frontalface_default.xml')
classifier =load_model('/home/harshit/Downloads/EMOTION-BASED-MUSIC-AND-VIDEO-RECOMMENDATION-SYSTEM-master/fer.h5')
...
def playlistmanager():
        ...
        try:
            ...
            if input=='Happy':
                if option=="video":
                    playlist_id=secrets.happy
                else:
                    playlist_id=secrets.mhappy
                
            elif input=='Sad':
                if option=="video":
                    playlist_id=secrets.sad
                else:
                    playlist_id=secrets.msad
            elif input=='Neutral':
                if option=="video":
                    playlist_id=secrets.neutral
                else:
                    playlist_id=secrets.mneutral
            elif input=='Angry':
                if option=="video":
                    playlist_id=secrets.angry
                else:
                    playlist_id=secrets.mangry
            elif input=='Surprise':
                if option=="video":
                    playlist_id=secrets.surprise
                else:
                    playlist_id=secrets.msurprise
            elif input=='Fear':
                if option=="video":
                    playlist_id=secrets.fear
                else:            
                    playlist_id=secrets.mfear
            else:
